string[] weapon = { "magic wand", "plastic fork", "banana", "sword", "brick" };
...
int villainStrikeStrength = villainsHP;

Console.WriteLine($"Today {villain} ({villainsHP} hp) saves the day with a {villainWeapon}!");

while(heroHP > 0 && villainsHP > 0)
{
    heroHP = heroHP - Hit(villain, villainStrikeStrength);
    villainsHP = villainsHP - Hit(hero, heroStrikeStrength);
}

Console.WriteLine($"Hero {hero} HP: {heroHP}");
Console.WriteLine($"Villain {villain} HP: {villainsHP}");
